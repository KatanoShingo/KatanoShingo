...
専門学校在学中に制作したポートフォリオ作品です。プロジェクトデータは紛失しており、当時のプレイ動画のみ残っています。

- 🥊 **[ミズターファイト](https://x.com/shi_k_7/status/1021315186248912898)**（2018）
  - **なにができる**: Unity 製のポートフォリオゲーム（学生時代）
  - **担当**: 企画〜開発
  - **残っているもの**: [プレイ動画（X）](https://x.com/shi_k_7/status/1021315186248912898)
...
- 🎁 **[VRC懸賞](https://vrc-kensyou.makuake.org/)**
  - **なにができる**: BOOTHショップのX懸賞をまとめて応募できるサイト。当選するとその場でギフトコードが表示され、抽選発表やDM待ちが不要
  - **担当**: 企画〜開発〜公開まで一通り

- 📌 **[VRCコルクボード](https://vrc-corkboard.makuake.org/)**
  - **なにができる**: VRChat用のコルクボード壁紙メーカー。写真や付箋を配置してFHD/QHD/4Kの壁紙を作成・保存（画像は端末内のみ）
  - **担当**: 企画〜開発〜公開まで一通り
...
CAD 設計や電子工作で作ったガジェットです。制作の様子は X の投稿からご覧ください。

- 🔐 **[1000円スマートロック](https://x.com/shi_k_7/status/1347814410828406784)**（2021）
  - **なにができる**: 市販スマートロックの代替として自作した簡易スマートロック（原価1000円以内）
  - **担当**: 設計〜自作

- ☂️ **[傘ロック](https://x.com/shi_k_7/status/1450075482364387328)**（2021）
  - **なにができる**: 傘のロック解除ボタンを塞ぎ、鍵がないと開けられないようにする盗難防止パーツ
  - **担当**: 設計〜3Dプリント

- 🧴 **[アルコールディスペンサー × ハトムギ ジョイント](https://x.com/shi_k_7/status/1655750556005314561)**（2023）
  - **なにができる**: アルコールディスペンサーとハトムギ化粧水を合体させる接続パーツ
  - **担当**: 設計〜3Dプリント
...
- **【公式】基本設定**： **`.gitignore`** (無視指定) ｜ **`.gitattributes`** (属性定義) ｜ **`.gitconfig`** (Git本体設定)
- **【公式】応用・詳細**： **`.gitmodules`** (サブモジュール) ｜ **`.git-blame-ignore-revs`** (整形履歴無視) ｜ **`.mailmap`** (名寄せ) ｜ **`.git/info/exclude`** (自分専用の無視設定)
- **【自動化・拡張】**： **`.gitlab-ci.yml`** (GitLab CI) ｜ **`.github/`** (GitHub Actions等) ｜ **`.githooks/`** (特定操作で発動するスクリプト) ｜ **`.lfsconfig`** (LFSの特殊設定)
- **【慣習・ダミー】**： **`.gitkeep`** (空フォルダ維持) ｜ **`.gitmessage`** (コミットメッセージ雛形)
- **⚠️ Gitの心臓部**： **`.git/`** (全履歴フォルダ) ｜ **`.git`** (サブモジュール等では心臓部へのパスが書かれた「ファイル」に変化)
