...
async function fetchNowFocus(login, userId) {
  const since = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString();
  const query = `
    query($login: String!, $since: GitTimestamp!, $userId: ID!) {
      user(login: $login) {
        repositories(ownerAffiliations: OWNER, first: 50, orderBy: {field: PUSHED_AT, direction: DESC}, isFork: false) {
          nodes {
            ...
            defaultBranchRef {
              target {
                ... on Commit {
                  history(first: 1, since: $since, author: {id: $userId}) {
                    totalCount
                  }
                }
              }
  ...
  `;
  const data = await graphql(query, { login, since, userId });
  const rows = (data.user.repositories.nodes || [])
    ...
    .sort((a, b) => b.commits - a.commits || new Date(b.pushedAt) - new Date(a.pushedAt))
    .slice(0, 3);
  return rows;
}
...
function buildSvg(model) {
  ...
  const rankCy = 220;
  const rightPanelLeft = 452;
  const rightPanelWidth = 440;
  const communityRankDividerX = 720;
  const rankColumnRight = rightPanelLeft + rightPanelWidth;
  const rankCx = (communityRankDividerX + rankColumnRight) / 2;
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-label="${escapeXml(
  ...
  <rect x="28" y="116" width="414" height="220" rx="10" fill="none" class="line" stroke-width="1.5"/>
  <rect x="452" y="116" width="440" height="220" rx="10" fill="none" class="line" stroke-width="1.5"/>

  <text x="50" y="145" class="primary" font-size="23" font-weight="700">Activity</text>
  ...
  <text x="474" y="206" class="fg" font-size="14">• Owned Repos（所有）: ${formatNum(model.totalRepos)}</text>
  ...
  <text x="870" y="576" class="muted" font-size="12" text-anchor="end">generated by GitHub Actions</text>
</svg>`;
}
